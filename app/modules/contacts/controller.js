...
const updateContact = async(req, res) => {
    ...
    try {
        ...
        if (contact) {
            ...
            let dataId = contact.userIds.concat(dataArray);2
...
const deleteContacts = async (req, res) => {
    try {
        ...
        if (contacts) {
            const contactsDelete = await Contact.findByIdAndDelete(req.params.id);
            res.status(200).json({ message: 'contacts list deleted' });
        } else {
            res.status(404).json({ message: 'contacts list not found' });
        }
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: error.message });
